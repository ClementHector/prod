...
def config_files_exist(prod_env_context):
    ...
    prod_dir = os.path.join(config_dir, 'prods', prod_env_context['prod_name'], 'config')
...
""" % (prod_env_context['temp_dir'].name, prod_env_context['temp_dir'].name, 
       prod_env_context['temp_dir'].name, prod_env_context['temp_dir'].name))
    ...
    prod_env_context['mock_create_alias'] = mock.patch(
        ...
        side_effect=lambda sw_name, sw_ver, pkgs, alias_name=None: 
...
def activate_production_env(prod_env_context):
    ...
    with prod_env_context['mock_load_config_paths'], \
         prod_env_context['mock_set_env_vars'], \
         prod_env_context['mock_create_alias'], \
         prod_env_context['mock_validate_rez']:
        
        prod_env = ProductionEnvironment(prod_env_context['prod_name'], prod_env_context['logger'])
        prod_env.activate()
